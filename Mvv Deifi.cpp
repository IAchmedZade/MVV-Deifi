...
public:
	...
	bool OnUserCreate() override
	{
		...
		std::pair<int, int> position = std::pair<int, int>{ ScreenWidth() / 2, ScreenHeight() / 2 };
	...
	void DisplayData() {
		...
		if (!(graph.globalTime % 10)) {
			...
			DrawString(110, 40, std::to_string(graph.globalTime / 10), olc::RED, 2);
